...
;((g: any) => {
  ...
  function GME_registerSpotlightCommand(command: SpotlightCommand): void {
  ...
  class SpotlightUI extends HTMLElement {
    static TAG_NAME = TAG
    ...
    #registerBuiltinCommands(): void {
      this.#commands.push({
        ...
        action: () => {
          if (typeof g.GME_openLogViewer === 'function') g.GME_openLogViewer()
        },
      })
    }
  ...
  function GME_openSpotlight(): void {
    const el = document.querySelector(TAG) as SpotlightUI | null
    if (el?.open) el.open()
  }

  g.GME_openSpotlight = GME_openSpotlight
  g.GME_registerSpotlightCommand = GME_registerSpotlightCommand
