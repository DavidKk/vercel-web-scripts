...
const editorTypingsSourcePath = path.join(repoRoot, 'preset/src/editor-typings.d.ts')
...
/**
 * Vite plugin: watch editor-typings.d.ts and generate lib/tampermonkey-editor-typings.generated.ts
 * on every build (including watch). Next.js uses that file for editor typings and MAP LOCAL gm-globals.d.ts.
 * No extra watch script: preset build + typings are one.
 */
function editorTypingsPlugin() {
  function generateEditorTypings() {
    const content = readFileSync(editorTypingsSourcePath, 'utf8')
    const escaped = content.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${')
    const outDir = path.dirname(editorTypingsOutPath)
    if (!existsSync(outDir)) mkdirSync(outDir, { recursive: true })
    const output = `/**
 * Generated by preset/vite (editorTypingsPlugin) from preset/src/editor-typings.d.ts.
 * Do not edit. Run "pnpm run build:preset" or "pnpm run dev" to regenerate.
 */

export const editorTypingsSource = \`${escaped}\`
`
    const tmpPath = `${editorTypingsOutPath}.tmp`
    writeFileSync(tmpPath, output, 'utf8')
    renameSync(tmpPath, editorTypingsOutPath)
    // eslint-disable-next-line no-console -- typings regenerate debug
    console.log('[editor-typings] lib/tampermonkey-editor-typings.generated.ts')
  }
  return {
    name: 'editor-typings',
    buildStart(this: { addWatchFile: (id: string) => void }) {
      this.addWatchFile(editorTypingsSourcePath)
    },
    writeBundle() {
      generateEditorTypings()
    },
  }
}
