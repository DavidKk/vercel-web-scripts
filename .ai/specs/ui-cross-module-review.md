...
| **Preset 孤儿模块** | `string-tool`、`compiled-code-viewer`、`codemirror-editor` 未接入 bundle                                              | 单独决策：接入或归档                                          |
...
preset/src/ui/
  ...
  string-tool/       # ⚠️ 未接入 entry-ui.ts
  compiled-code-viewer/  # ⚠️ 未接入
  codemirror-editor/     # ⚠️ 未接入
  shared/
    vws-ui-tokens.css
    vws-scroll.css
    scroll-indicator.ts
...
- Preset：log-viewer、string-tool、compiled-code-viewer 各一套 BEM modal
...
#### 3.14 Preset 未打包 UI 模块

`string-tool`、`compiled-code-viewer`、`codemirror-editor` 已实现但未 import 进 `entry-ui.ts`。

**决策**：接入 preset-ui bundle，或移到 `experimental/` / 删除 dead code。
...
| **D** | Preset 孤儿模块决策                                        | —    | 避免 dead code 误导    |
