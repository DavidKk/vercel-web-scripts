...
MagickMonkey 主要用于在 Vercel 上管理和部署自定义脚本。
...
- **Shell 日志**：扩展 Popup 可选 Console / Log Viewer / Off（仅约束 preset 与 wrapper）；用户 GIST 脚本正文始终输出到控制台。油猴安装无 Logger 开关，默认全部在 DevTools 可见。
