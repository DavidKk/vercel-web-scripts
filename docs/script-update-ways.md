...
| **Launcher 菜单「Update Script」** | 用户在 Tampermonkey 菜单里点击「Update Script」                                            | 使用 launcher 的任意页面                                | Launcher 清空 preset 缓存 → 约 500ms 后 `location.reload()`，下次加载重新拉 preset 与远程脚本（全部强制更新） |
...
| Launcher「Update Script」 | Preset + 远程脚本             | 是                      | 任意      |
...
- **Launcher「Update Script」**: `services/tampermonkey/launcherScript.ts`（`GM_registerMenuCommand('Update Script', ...)`）
